...
function buildDevopsOptimizationModel(telemetry = buildPlatformTelemetryModel()) {
  ...
  const actions = [
    ...
    },
    {
      ...
      priority: 2,
      safeMode: true,
      message: `${candidates.length} deployment-like pods can be reviewed for worker node scheduling; V14 only reports candidates and does not mutate scheduling.`,
    },
    {
...
function buildRiskVectors(evidence, ledger, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  ...
  if (devops.summary.controlPodShare >= 70) add('devops-control-plane-overcommit', Math.min(94, 58 + devops.summary.controlPodShare / 2), 'runtime', 'devops-pod-density', `${devops.summary.controlPodShare}% of observed pods are on control-plane; review ${devops.summary.statelessCandidateTotal} stateless candidates before tuning resources`, 'v14-live');
  if (devops.summary.workerIdle) add('worker-capacity-idle', 78, 'runtime', 'worker-placement-balance', `${devops.summary.workerPods}/${devops.summary.totalPods} pods are on worker nodes while ${devops.summary.workerReady}/${devops.summary.workerTotal} workers are Ready`, 'v14-live');
  if (devops.summary.metricsScrapeTimeout) add('metrics-scrape-devops-timeout', 80, 'observability', 'devops-resource-sizing', 'Resource right-sizing is not trustworthy until metrics scrape/top data is stable', 'v14-live');
...
function calculateScores(evidence, ledger, risks, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  ...
  const intelligenceScore = clamp((Number(s.platformHealthScore || 80) * .18) + (serviceRate * .15) + (podRate * .13) + (riskScore * .14) + (avgStage * .10) + (dataPlatformScore * .15) + (telemetryScore * .07) + (devopsOptimizationScore * .08));
  return {
...
function buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  const timestamp = now();
...
function buildMetrics(evidence, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  ...
  lines.push('# HELP cicd_v14_devops_optimization_score V14 DevOps placement and stability optimization score.');
  lines.push('# TYPE cicd_v14_devops_optimization_score gauge');
  lines.push(`cicd_v14_devops_optimization_score{build="${metricLabel(build)}"} ${devops.summary.score}`);
  lines.push('# HELP cicd_v14_node_pod_count Pod count by node observed by the V14 optimizer.');
  lines.push('# TYPE cicd_v14_node_pod_count gauge');
  for (const node of devops.nodes) lines.push(`cicd_v14_node_pod_count{node="${metricLabel(node.node)}",role="${metricLabel(node.role)}",build="${metricLabel(build)}"} ${node.pods}`);
  lines.push('# HELP cicd_v14_devops_migration_candidates Deployment-like control-plane pods that can be reviewed for worker scheduling.');
  lines.push('# TYPE cicd_v14_devops_migration_candidates gauge');
  lines.push(`cicd_v14_devops_migration_candidates{build="${metricLabel(build)}"} ${devops.summary.statelessCandidateTotal}`);
  lines.push('# HELP cicd_v14_metrics_scrape_timeout Whether V14 saw metrics scrape/top timeout risk.');
  lines.push('# TYPE cicd_v14_metrics_scrape_timeout gauge');
  lines.push(`cicd_v14_metrics_scrape_timeout{build="${metricLabel(build)}"} ${devops.summary.metricsScrapeTimeout ? 1 : 0}`);
  return lines.join('\n') + '\n';
}
...
function buildGrafana(scores, dataPlatform = { summary: {} }, telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  const panels = [];
  panels.push(textPanel('V14 Intelligence Brief', 0, 0, 24, 3, `### ZhangLab V14 Platform Intelligence\n\nBuild #${build} · ${semver} · commit ${commit}. V14 now treats Spark, Flink, Kafka, Airflow, Trino, Superset and MinIO as a first-class data application lab: ${dataPlatform.summary.readyApps || 0}/${dataPlatform.summary.observedApps || 0} observed apps ready, ${dataPlatform.summary.readyFlows || 0}/${dataPlatform.summary.flowTotal || 0} flows ready. DevOps optimization: score=${devops.summary.score}, controlPods=${devops.summary.controlPlanePods}, workerPods=${devops.summary.workerPods}, candidates=${devops.summary.statelessCandidateTotal}. Upgrade telemetry gate: metrics=${telemetry.summary.metricsStatus}, timeoutEvents=${telemetry.summary.timeoutEvents}, versionSkew=${telemetry.summary.versionSkewDetected}.`));
  ...
  panels.push(statPanel('DevOps Optimization Score', 16, 3, 4, 4, `cicd_v14_devops_optimization_score{build="${build}"}`, { min: 0, max: 100 }));
  ...
  const graphPanels = [
    ...
    ['bargauge', 'DevOps Node Pod Distribution', 12, 15, 12, 8, [promTarget(`cicd_v14_node_pod_count{build="${build}"}`, '{{role}} {{node}}', 'A', 'table')], { decimals: 0, panelOptions: { displayMode: 'gradient', orientation: 'horizontal', showUnfilled: true } }],
...
function buildPortal(evidence, ledger, risks, playbooks, scores, telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  ...
  const evidenceJson = JSON.stringify({ summary: { build, semver, commit, scores, inherited: evidence.summary || {}, risks: risks.length, playbooks: playbooks.length, dataPlatform: dataPlatform.summary, telemetry: telemetry.summary, devops: devops.summary }, risks, playbooks, ledger, dataPlatform, telemetry, devops }).replace(/</g, '\\u003c');
  return `<!doctype html>
...
function buildAll() {
  ...
  const events = buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform, telemetry, devops);
  const summary = { '@timestamp': now(), pipeline_version: 'v14', build, job, commit, semver, publicUrl: `https://${publicHost}/`, portalUrl: `http://192.168.1.58:${portalNodePort}/`, inheritedV13Build: evidence.summary?.build || 'unknown', scores, dataPlatform: dataPlatform.summary, telemetry: telemetry.summary, devops: devops.summary, riskVectorTotal: risks.length, playbookTotal: playbooks.length, stageLedgerTotal: ledger.length, v13Summary: evidence.summary || {} };
  write('reports/v14-intelligence/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform, telemetry, devops, inherited: evidence }, null, 2) + '\n');
  ...
  write('reports/v14-devops-optimization.json', JSON.stringify(devops, null, 2) + '\n');
  write('reports/v14-devops-optimization.ndjson', [devops.summary, ...devops.nodes, ...devops.candidates, ...devops.actions].map((item) => JSON.stringify({ '@timestamp': now(), pipeline_version: 'v14', build, job, commit, ...item, type: item.node ? 'devops_node_snapshot' : item.pod ? 'devops_candidate_snapshot' : item.key ? 'devops_action_snapshot' : 'devops_summary_snapshot' })).join('\n') + '\n');
  ...
  write('reports/v14-prometheus-metrics.prom', buildMetrics(evidence, risks, playbooks, scores, dataPlatform, telemetry, devops));
  write('reports/v14-grafana-dashboard.json', JSON.stringify(buildGrafana(scores, dataPlatform, telemetry, devops), null, 2) + '\n');
  write('reports/v14-kibana-dashboard.ndjson', buildKibana().map((object) => JSON.stringify(object)).join('\n') + '\n');
  write('reports/v14-portal/index.html', buildPortal(evidence, ledger, risks, playbooks, scores, telemetry, devops));
  write('reports/v14-portal/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform, telemetry, devops }, null, 2) + '\n');
  console.log(`v14_build_complete events=${events.length} stages=${ledger.length} risks=${risks.length} score=${scores.intelligenceScore} data_score=${scores.dataPlatformScore} devops_score=${scores.devopsOptimizationScore}`);
}

function lintAll() {
  const required = ['reports/v14-intelligence/evidence.json','reports/v14-data-platform-apps.json','reports/v14-data-platform-apps.ndjson','reports/v14-platform-telemetry.json','reports/v14-platform-telemetry.ndjson','reports/v14-devops-optimization.json','reports/v14-devops-optimization.ndjson','reports/v14-risk-score.json','reports/v14-recovery-playbooks.json','reports/v14-observability.ndjson','reports/v14-prometheus-metrics.prom','reports/v14-grafana-dashboard.json','reports/v14-kibana-dashboard.ndjson','reports/v14-portal/index.html','reports/v14-portal/evidence.json'];
  ...
  for (const needle of ['jenkins-v14-intelligence-es', 'state-timeline', 'status-history', 'nodeGraph', 'Big Data App Readiness', 'Big Data Flow State', 'Telemetry Upgrade Score', 'Probe Timeout Events', 'DevOps Optimization Score', 'DevOps Node Pod Distribution']) if (!dashboardText.includes(needle)) throw new Error(`grafana missing ${needle}`);
  ...
  for (const needle of ['zhanglab-platform-pipeline-observability', 'zhanglab-platform-observability-universal-command', 'Universal Observability Command', '通用流水线评分总览', '通用 DevOps 节点分布', '通用 DevOps 优化台账', '通用大数据应用就绪度', '通用大数据链路证据']) if (!kibanaText.includes(needle)) throw new Error(`kibana missing ${needle}`);
  const html = fs.readFileSync('reports/v14-portal/index.html', 'utf8');
  for (const needle of ['v14-intelligence-hypergrid-20260601', 'v14-sonic-command-20260601', 'sound-rev-v2', 'V14 Data Lab', 'Data Command', 'Big Data Application Lab', 'Data Flow Evidence Map', 'DevOps Optimize', 'DevOps Node Pressure', 'Stateless Migration Candidates', '升级体检门禁', 'Metrics API', 'Risk Radar', 'Recovery Room', 'Command Matrix', 'AudioContext', 'Sound ON', 'pointerdown', 'keydown']) if (!html.includes(needle)) throw new Error(`portal missing ${needle}`);
