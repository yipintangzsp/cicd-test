...
def notifyDingTalk(String status, String message) {
    try {
        ...
        writeFile file: '.v9-dingtalk-payload.json', text: payload
        sh '''
            set +e
            curl -sS -m 8 -H 'Content-Type: application/json' \
              -X POST \
              --data-binary @.v9-dingtalk-payload.json \
              'http://192.168.1.58:18080/dingtalk' \
              >/tmp/v9-dingtalk-notify.out 2>&1
            if [ $? -ne 0 ]; then
              echo "DingTalk relay notify failed, ignored"
              cat /tmp/v9-dingtalk-notify.out || true
            fi
            rm -f .v9-dingtalk-payload.json
        '''
    } catch (e) {
...
""".stripIndent()
    ...
    withCredentials([usernamePassword(
        ...
        sh '''
            ...
            if echo "$LAST_MSG" | grep -q 'ci(v9)'; then
