...
function buildRiskVectors(evidence, ledger, dataPlatform = buildDataPlatformModel(evidence)) {
  const s = evidence.summary || {};
  const risks = [];
...
function buildPlaybooks(evidence, risks, dataPlatform = buildDataPlatformModel(evidence)) {
  const base = [
...
function calculateScores(evidence, ledger, risks, dataPlatform = buildDataPlatformModel(evidence)) {
  const s = evidence.summary || {};
  ...
  const intelligenceScore = clamp((Number(s.platformHealthScore || 80) * .23) + (serviceRate * .18) + (podRate * .16) + (riskScore * .16) + (avgStage * .12) + (dataPlatformScore * .15));
  return {
...
function buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence)) {
  const timestamp = now();
  ...
  const events = [];
  events.push({ ...baseFields, type: 'v14_summary', score: scores.intelligenceScore, risk_score: scores.riskScore, health_score: scores.platformHealth, recovery_score: scores.recoveryScore, observability_score: scores.observabilityScore, data_platform_score: scores.dataPlatformScore, complexity_lift_percent: scores.complexityLiftPercent, message: 'V14 intelligence summary' });
...
function buildMetrics(evidence, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence)) {
  const lines = [];
...
function buildGrafana(scores, dataPlatform = { summary: {} }) {
  const panels = [];
  panels.push(textPanel('V14 Intelligence Brief', 0, 0, 24, 3, `### ZhangLab V14 Platform Intelligence\n\nBuild #${build} · ${semver} · commit ${commit}. V14 now treats Spark, Flink, Kafka, Airflow, Trino, Superset and MinIO as a first-class data application lab: ${dataPlatform.summary.readyApps || 0}/${dataPlatform.summary.observedApps || 0} observed apps ready, ${dataPlatform.summary.readyFlows || 0}/${dataPlatform.summary.flowTotal || 0} flows ready.`));
...
function buildKibana() {
  ...
  const vis = [
    ...
    legacyVis('operations-ledger', '通用运行记录', 'table', [metricAgg(), termsAgg('2','pipeline_version','bucket',10), termsAgg('3','type','bucket',20), termsAgg('4','category','bucket',20), termsAgg('5','status','bucket',10)], 'pipeline_version : *'),
  ];
  const dashboards = [
...
function buildPortal(evidence, ledger, risks, playbooks, scores) {
  ...
  const evidenceJson = JSON.stringify({ summary: { build, semver, commit, scores, inherited: evidence.summary || {}, risks: risks.length, playbooks: playbooks.length, dataPlatform: dataPlatform.summary }, risks, playbooks, ledger, dataPlatform }).replace(/</g, '\\u003c');
  return `<!doctype html>
...
function buildAll() {
  ...
  const dataPlatform = buildDataPlatformModel(evidence);
  const risks = buildRiskVectors(evidence, ledger, dataPlatform);
  const playbooks = buildPlaybooks(evidence, risks, dataPlatform);
  const scores = calculateScores(evidence, ledger, risks, dataPlatform);
  const events = buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform);
  const summary = { '@timestamp': now(), pipeline_version: 'v14', build, job, commit, semver, publicUrl: `https://${publicHost}/`, portalUrl: `http://192.168.1.58:${portalNodePort}/`, inheritedV13Build: evidence.summary?.build || 'unknown', scores, dataPlatform: dataPlatform.summary, riskVectorTotal: risks.length, playbookTotal: playbooks.length, stageLedgerTotal: ledger.length, v13Summary: evidence.summary || {} };
  write('reports/v14-intelligence/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform, inherited: evidence }, null, 2) + '\n');
  ...
  write('reports/v14-prometheus-metrics.prom', buildMetrics(evidence, risks, playbooks, scores, dataPlatform));
  write('reports/v14-grafana-dashboard.json', JSON.stringify(buildGrafana(scores, dataPlatform), null, 2) + '\n');
  ...
  write('reports/v14-portal/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform }, null, 2) + '\n');
...
function lintAll() {
  ...
  for (const needle of ['jenkins-v14-intelligence-es', 'state-timeline', 'status-history', 'nodeGraph', 'Big Data App Readiness', 'Big Data Flow State']) if (!dashboardText.includes(needle)) throw new Error(`grafana missing ${needle}`);
  ...
  for (const needle of ['zhanglab-platform-pipeline-observability', 'zhanglab-platform-observability-universal-command', 'Universal Observability Command', '通用流水线评分总览', '通用运行记录', '通用大数据应用就绪度', '通用大数据链路证据']) if (!kibanaText.includes(needle)) throw new Error(`kibana missing ${needle}`);
  const html = fs.readFileSync('reports/v14-portal/index.html', 'utf8');
  for (const needle of ['v14-intelligence-hypergrid-20260601', 'v14-sonic-command-20260601', 'sound-rev-v2', 'V14 Data Lab', 'Data Command', 'Big Data Application Lab', 'Data Flow Evidence Map', 'Risk Radar', 'Recovery Room', 'Command Matrix', 'AudioContext', 'Sound ON', 'pointerdown', 'keydown']) if (!html.includes(needle)) throw new Error(`portal missing ${needle}`);
