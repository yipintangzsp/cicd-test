...
function lintAll() {
  ...
  if (kibanaLines.length !== 8) throw new Error(`kibana should contain 1 universal data view, 6 visualizations and 1 dashboard, got: ${kibanaLines.length}`);
  ...
  for (const needle of ['zhanglab-platform-pipeline-observability', 'zhanglab-platform-observability-universal-command', 'Universal Observability Command', '通用流水线评分总览', '通用运行记录']) if (!kibanaText.includes(needle)) throw new Error(`kibana missing ${needle}`);
