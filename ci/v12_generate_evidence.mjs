...
const portalNodePort = process.env.V12_PORTAL_NODEPORT || '30087';
const publicHost = process.env.CLOUDFLARE_PUBLIC_HOSTNAME || 'platform.heil.ccwu.cc';
const grafanaTitle = process.env.V12_GRAFANA_DASHBOARD_TITLE || 'Jenkins V12 Platform Evidence';
const kibanaIndexPrefix = process.env.V12_KIBANA_INDEX_PREFIX || 'jenkins-v12-platform';
...
const podCoverageSummary = readJson('reports/v12-cluster-resilience/pod-full-coverage-summary.json', {});
const serviceProbes = readLines('reports/v12-service-probes.ndjson').map((line) => JSON.parse(line));
...
ensureDir('reports/v12-portal');
write('reports/v12-platform-services.json', JSON.stringify(evidence, null, 2));
write('reports/v12-portal/evidence.json', JSON.stringify(evidence, null, 2));

const ndjson = [];
ndjson.push(JSON.stringify({ '@timestamp': now, type: 'platform_summary', pipeline_version: 'v12', pipeline_result_key: platformHealthScore >= 90 ? 'EXCELLENT' : platformHealthScore >= 75 ? 'HEALTHY' : 'ATTENTION', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: 'all', health_score: platformHealthScore, risk_score: Math.max(0, 100 - platformHealthScore), ...summary }));
ndjson.push(JSON.stringify({ '@timestamp': now, type: 'pod_coverage_summary', pipeline_version: 'v12', pipeline_result_key: podCoverageComplete && podCoverageNotHealthy === 0 ? 'FULL_COVERAGE' : 'ATTENTION', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: 'cluster-core', health_score: podCoverageRatio, risk_score: podCoverageComplete ? podCoverageNotHealthy * 10 : 80, ...summary }));
for (const row of podRows) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'pod', pipeline_version: 'v12', pipeline_result_key: row.ready ? 'READY' : 'NOT_READY', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: layerFor(row.namespace, row.name), health_score: row.ready ? Math.max(55, 100 - Math.min(45, row.restarts * 5)) : 15, risk_score: row.ready ? Math.min(70, row.restarts * 8) : 90, ready_count: row.ready ? 1 : 0, coverage_count: 1, ...row }));
for (const row of svcRows) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'service', pipeline_version: 'v12', pipeline_result_key: row.endpointReady > 0 || row.type === 'ExternalName' ? 'READY' : 'NO_ENDPOINT', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: layerFor(row.namespace, row.name), health_score: row.endpointReady > 0 || row.type === 'ExternalName' ? 100 : 20, risk_score: row.endpointReady > 0 || row.type === 'ExternalName' ? 0 : 70, ...row }));
for (const row of serviceProbes) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'service_probe', pipeline_version: 'v12', pipeline_result_key: row.status === 'ok' ? 'SUCCESS' : 'FAILURE', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: layerFor(row.namespace, row.name), health_score: row.status === 'ok' ? 100 : 0, risk_score: row.status === 'ok' ? 0 : 75, ok_count: row.status === 'ok' ? 1 : 0, failed_count: row.status === 'ok' ? 0 : 1, ...row }));
for (const row of namespaceSummary) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'namespace_summary', pipeline_version: 'v12', pipeline_result_key: row.ready >= row.pods ? 'READY' : 'ATTENTION', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: layerFor(row.namespace), health_score: pct(row.ready, row.pods), risk_score: Math.max(0, 100 - pct(row.ready, row.pods)) + Math.min(30, row.restarts), ...row }));
for (const row of nodeRows) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'node', pipeline_version: 'v12', pipeline_result_key: row.ready ? 'READY' : 'NOT_READY', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: 'cluster-core', health_score: row.ready ? 100 : 0, risk_score: row.ready ? 0 : 95, ...row }));
for (const row of layerSummary) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'layer_summary', pipeline_version: 'v12', pipeline_result_key: row.healthScore >= 90 ? 'EXCELLENT' : row.healthScore >= 75 ? 'HEALTHY' : 'ATTENTION', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: row.layer, health_score: row.healthScore, risk_score: Math.max(0, 100 - row.healthScore), ...row }));
for (const row of jobMarketFit) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'capability_fit', pipeline_version: 'v12', pipeline_result_key: row.score >= 90 ? 'STRONG' : row.score >= 75 ? 'GOOD' : 'GAP', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, layer: 'market-fit', capability: row.capability, demand: row.demand, coverage: row.coverage, health_score: row.score, risk_score: Math.max(0, 100 - row.score), score: row.score, platform: row.platform }));
for (const row of riskEvents) ndjson.push(JSON.stringify({ '@timestamp': now, type: 'risk_event', pipeline_version: 'v12', pipeline_result_key: row.severity >= 85 ? 'CRITICAL' : 'WARNING', pipeline_job_key: job, pipeline_build_number: Number(build) || 0, build, health_score: Math.max(0, 100 - row.severity), risk_score: row.severity, ...row }));
write('reports/v12-observability.ndjson', ndjson.join('\n') + '\n');

const metrics = [
  '# HELP cicd_v12_pod_ready Kubernetes pod readiness captured by Jenkins v12.',
  '# TYPE cicd_v12_pod_ready gauge',
];
for (const row of podRows) metrics.push(`cicd_v12_pod_ready{namespace="${metricLabel(row.namespace)}",pod="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.ready ? 1 : 0}`);
metrics.push('# HELP cicd_v12_pod_coverage Kubernetes pod coverage records captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_pod_coverage gauge');
for (const row of podRows) metrics.push(`cicd_v12_pod_coverage{namespace="${metricLabel(row.namespace)}",pod="${metricLabel(row.name)}",node="${metricLabel(row.node)}",build="${metricLabel(build)}"} 1`);
metrics.push('# HELP cicd_v12_pod_coverage_ratio Kubernetes live pod coverage ratio captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_pod_coverage_ratio gauge');
metrics.push(`cicd_v12_pod_coverage_ratio{build="${metricLabel(build)}"} ${podCoverageRatio}`);
metrics.push('# HELP cicd_v12_pod_restarts Kubernetes pod restart count captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_pod_restarts gauge');
for (const row of podRows) metrics.push(`cicd_v12_pod_restarts{namespace="${metricLabel(row.namespace)}",pod="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.restarts}`);
metrics.push('# HELP cicd_v12_service_endpoint_ready Ready endpoints captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_service_endpoint_ready gauge');
for (const row of svcRows) metrics.push(`cicd_v12_service_endpoint_ready{namespace="${metricLabel(row.namespace)}",service="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.endpointReady}`);
metrics.push('# HELP cicd_v12_service_probe_ok Platform service probe status captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_service_probe_ok gauge');
for (const row of serviceProbes) metrics.push(`cicd_v12_service_probe_ok{group="${metricLabel(row.group)}",namespace="${metricLabel(row.namespace)}",service="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.status === 'ok' ? 1 : 0}`);
metrics.push('# HELP cicd_v12_spark_component_ready Spark component readiness captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_spark_component_ready gauge');
for (const row of sparkPods) metrics.push(`cicd_v12_spark_component_ready{namespace="${metricLabel(row.namespace)}",component="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.ready ? 1 : 0}`);
metrics.push('# HELP cicd_v12_node_ready Kubernetes node readiness captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_node_ready gauge');
for (const row of nodeRows) metrics.push(`cicd_v12_node_ready{node="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.ready ? 1 : 0}`);
metrics.push('# HELP cicd_v12_market_capability_score Recruitment capability fit score captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_market_capability_score gauge');
for (const row of jobMarketFit) metrics.push(`cicd_v12_market_capability_score{capability="${metricLabel(row.capability)}",coverage="${metricLabel(row.coverage)}",build="${metricLabel(build)}"} ${row.score}`);
metrics.push('# HELP cicd_v12_platform_health_score Composite platform health score captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_platform_health_score gauge');
metrics.push(`cicd_v12_platform_health_score{build="${metricLabel(build)}"} ${platformHealthScore}`);
metrics.push('# HELP cicd_v12_layer_health_score Composite layer health score captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_layer_health_score gauge');
for (const row of layerSummary) metrics.push(`cicd_v12_layer_health_score{layer="${metricLabel(row.layer)}",build="${metricLabel(build)}"} ${row.healthScore}`);
metrics.push('# HELP cicd_v12_risk_event_severity Risk event severity captured by Jenkins v12.');
metrics.push('# TYPE cicd_v12_risk_event_severity gauge');
for (const row of riskEvents.slice(0, 30)) metrics.push(`cicd_v12_risk_event_severity{category="${metricLabel(row.category)}",layer="${metricLabel(row.layer)}",namespace="${metricLabel(row.namespace)}",name="${metricLabel(row.name)}",build="${metricLabel(build)}"} ${row.severity}`);
write('reports/v12-prometheus-metrics.prom', metrics.join('\n') + '\n');

const grafana = {
  title: grafanaTitle,
  tags: ['jenkins', 'v12', 'platform', 'spark'],
  timezone: 'browser',
  schemaVersion: 39,
  version: 1,
  refresh: '30s',
  panels: [
    { type: 'stat', title: 'Platform Health Score', gridPos: { x: 0, y: 0, w: 4, h: 4 }, targets: [{ expr: 'cicd_v12_platform_health_score' }] },
    { type: 'stat', title: 'Ready Pods', gridPos: { x: 4, y: 0, w: 4, h: 4 }, targets: [{ expr: 'sum(cicd_v12_pod_ready)' }] },
    { type: 'stat', title: 'Service Probes OK', gridPos: { x: 8, y: 0, w: 4, h: 4 }, targets: [{ expr: 'sum(cicd_v12_service_probe_ok)' }] },
    { type: 'stat', title: 'Spark Ready', gridPos: { x: 12, y: 0, w: 4, h: 4 }, targets: [{ expr: 'sum(cicd_v12_spark_component_ready)' }] },
    { type: 'stat', title: 'Risk Events', gridPos: { x: 16, y: 0, w: 4, h: 4 }, targets: [{ expr: 'count(cicd_v12_risk_event_severity)' }] },
    { type: 'stat', title: 'Ready Nodes', gridPos: { x: 20, y: 0, w: 4, h: 4 }, targets: [{ expr: 'sum(cicd_v12_node_ready)' }] },
    { type: 'gauge', title: 'Layer Health Gauge', gridPos: { x: 0, y: 4, w: 8, h: 8 }, targets: [{ expr: 'avg(cicd_v12_layer_health_score)' }] },
    { type: 'bargauge', title: 'Layer Health Ranking', gridPos: { x: 8, y: 4, w: 16, h: 8 }, targets: [{ expr: 'cicd_v12_layer_health_score' }] },
    { type: 'timeseries', title: 'Pod Readiness By Namespace', gridPos: { x: 0, y: 12, w: 12, h: 8 }, targets: [{ expr: 'sum by(namespace) (cicd_v12_pod_ready)' }] },
    { type: 'timeseries', title: 'Service Probe OK By Group', gridPos: { x: 12, y: 12, w: 12, h: 8 }, targets: [{ expr: 'sum by(group) (cicd_v12_service_probe_ok)' }] },
    { type: 'barchart', title: 'Restart Hotspots', gridPos: { x: 0, y: 20, w: 12, h: 8 }, targets: [{ expr: 'topk(20, cicd_v12_pod_restarts)' }] },
    { type: 'barchart', title: 'Risk Severity Hotspots', gridPos: { x: 12, y: 20, w: 12, h: 8 }, targets: [{ expr: 'topk(20, cicd_v12_risk_event_severity)' }] },
    { type: 'table', title: 'Spark Components', gridPos: { x: 0, y: 28, w: 12, h: 8 }, targets: [{ expr: 'cicd_v12_spark_component_ready' }] },
    { type: 'barchart', title: 'Recruitment Capability Fit', gridPos: { x: 12, y: 28, w: 12, h: 8 }, targets: [{ expr: 'cicd_v12_market_capability_score' }] },
    { type: 'table', title: 'Service Probe Detail', gridPos: { x: 0, y: 36, w: 12, h: 8 }, targets: [{ expr: 'cicd_v12_service_probe_ok' }] },
    { type: 'table', title: 'Layer Health Detail', gridPos: { x: 12, y: 36, w: 12, h: 8 }, targets: [{ expr: 'cicd_v12_layer_health_score' }] },
    { type: 'stat', title: 'Full Pod Coverage Ratio', gridPos: { x: 0, y: 44, w: 6, h: 4 }, targets: [{ expr: 'cicd_v12_pod_coverage_ratio' }] },
    { type: 'table', title: 'Full Pod Coverage Records', gridPos: { x: 6, y: 44, w: 18, h: 8 }, targets: [{ expr: 'cicd_v12_pod_coverage' }] },
  ],
};
write('reports/v12-grafana-dashboard.json', JSON.stringify(grafana, null, 2));

function legacyVis(id, title, visType, aggs, kql = 'pipeline_version : v12') {
  return {
    type: 'visualization',
    id: `${kibanaIndexPrefix}-${id}`,
    attributes: {
      title,
      description: 'Generated by Jenkins V12 for platform evidence observability.',
...
const kibanaVisualizations = [
  legacyVis('platform-health-metric', 'V12 平台综合健康分', 'metric', [avgHealthAgg], 'type : platform_summary'),
...
const kibanaObjects = [
  ...
  dashboardObject('evidence', `${grafanaTitle} · Command`, 'V12 command dashboard with health score, layer status, probes, capability fit and trend.', overviewVisualizations, 'pipeline_version : v12'),
  dashboardObject('risk-response', `${grafanaTitle} · Risk Response`, 'Focused risk dashboard for failed probes, restart hotspots, endpoint gaps and namespace attention.', riskVisualizations, 'pipeline_version : v12 and (type : risk_event or type : namespace_summary or type : service_probe or type : platform_summary)'),
  dashboardObject('data-flow', `${grafanaTitle} · Data Flow`, 'Focused data platform dashboard for Spark, Kafka, Flink, Airflow, Trino, Superset and MinIO evidence.', dataFlowVisualizations, 'pipeline_version : v12 and (layer : data-platform or type : capability_fit or type : platform_summary)'),
];
write('reports/v12-kibana-dashboard.ndjson', kibanaObjects.map((object) => JSON.stringify(object)).join('\n') + '\n');
...
<head>
  ...
  <title>ZhangLab DevOps V12 Control Surface</title>
  <style>
...
<body>
  <header>
    <div>
      <h1>ZhangLab DevOps V12 Control Surface</h1>
...
write('reports/v12-portal/index.html', html);
