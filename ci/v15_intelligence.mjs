...
function ensureStageLedger() {
  if (readLines('reports/v15-intelligence/stage-ledger.ndjson').length) return;
...
function calculateScores(evidence, ledger, risks, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry)) {
  ...
  const intelligenceScore = clamp((Number(s.platformHealthScore || 80) * .12) + (serviceRate * .11) + (podRate * .10) + (riskScore * .11) + (avgStage * .08) + (dataPlatformScore * .13) + (telemetryScore * .06) + (devopsOptimizationScore * .07) + (autonomousOpsScore * .14) + (bigDataDepthScore * .08));
  return {
...
function buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry), autonomous = buildAutonomousOpsModel(evidence, dataPlatform, telemetry, devops, risks, ledger)) {
  const timestamp = now();
...
function buildMetrics(evidence, risks, playbooks, scores, dataPlatform = buildDataPlatformModel(evidence), telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry), autonomous = buildAutonomousOpsModel(evidence, dataPlatform, telemetry, devops, risks, [])) {
  const lines = [];
...
function buildGrafana(scores, dataPlatform = { summary: {} }, telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry), autonomous = buildAutonomousOpsModel({}, dataPlatform, telemetry, devops, [], [])) {
  const panels = [];
  panels.push(textPanel('V15 Intelligence Brief', 0, 0, 24, 3, `### ZhangLab V15 Autonomous DataOps Intelligence\n\nBuild #${build} · ${semver} · commit ${commit}. V15 keeps V14 as the evidence spine, then adds autonomous operation score=${autonomous.summary.score}, incident readiness=${autonomous.summary.incidentReadinessScore}, FinOps=${autonomous.summary.finopsScore}, big-data depth=${autonomous.summary.dataDepthScore}. Spark/Flink/Kafka/Airflow/Trino/Superset/MinIO remain first-class data apps: ${dataPlatform.summary.readyApps || 0}/${dataPlatform.summary.observedApps || 0} observed apps ready, ${dataPlatform.summary.readyFlows || 0}/${dataPlatform.summary.flowTotal || 0} flows ready. No destructive actions are embedded.`));
  ...
  panels.push(statPanel('Node Ready', 21, 3, 3, 4, 'sum(kube_node_status_condition{condition="Ready",status="true"})', { decimals: 0 }));
  const graphPanels = [
    ...
    ['stat', 'Telemetry Upgrade Score', 0, 56, 8, 5, [promTarget(`cicd_v15_platform_telemetry_score{build="${build}"}`, 'telemetry')], { min: 0, max: 100, panelOptions: { colorMode: 'background', graphMode: 'area', justifyMode: 'center', reduceOptions: { calcs: ['lastNotNull'], fields: '', values: false } } }],
...
function buildPortal(evidence, ledger, risks, playbooks, scores, telemetry = buildPlatformTelemetryModel(), devops = buildDevopsOptimizationModel(telemetry), autonomous = buildAutonomousOpsModel(evidence, buildDataPlatformModel(evidence), telemetry, devops, risks, ledger)) {
  ...
  const evidenceJson = JSON.stringify({ summary: { build, semver, commit, scores, inherited: evidence.summary || {}, risks: risks.length, playbooks: playbooks.length, dataPlatform: dataPlatform.summary, telemetry: telemetry.summary, devops: devops.summary, autonomous: autonomous.summary }, risks, playbooks, ledger, dataPlatform, telemetry, devops, autonomous }).replace(/</g, '\\u003c');
  return `<!doctype html>
...
function buildAll() {
  ...
  const autonomous = buildAutonomousOpsModel(evidence, dataPlatform, telemetry, devops, risks, ledger);
  const events = buildObservabilityEvents(evidence, ledger, risks, playbooks, scores, dataPlatform, telemetry, devops, autonomous);
  const summary = { '@timestamp': now(), pipeline_version: 'v15', build, job, commit, semver, publicUrl: `https://${publicHost}/`, portalUrl: `http://192.168.1.58:${portalNodePort}/`, inheritedV14Build: evidence.summary?.build || 'unknown', scores, dataPlatform: dataPlatform.summary, telemetry: telemetry.summary, devops: devops.summary, autonomous: autonomous.summary, riskVectorTotal: risks.length, playbookTotal: playbooks.length, stageLedgerTotal: ledger.length, v14Summary: evidence.summary || {} };
  write('reports/v15-intelligence/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform, telemetry, devops, autonomous, inherited: evidence }, null, 2) + '\n');
  ...
  write('reports/v15-prometheus-metrics.prom', buildMetrics(evidence, risks, playbooks, scores, dataPlatform, telemetry, devops, autonomous));
  write('reports/v15-grafana-dashboard.json', JSON.stringify(buildGrafana(scores, dataPlatform, telemetry, devops, autonomous), null, 2) + '\n');
  write('reports/v15-kibana-dashboard.ndjson', buildKibana().map((object) => JSON.stringify(object)).join('\n') + '\n');
  write('reports/v15-portal/index.html', buildPortal(evidence, ledger, risks, playbooks, scores, telemetry, devops, autonomous));
  write('reports/v15-portal/evidence.json', JSON.stringify({ summary, scores, risks, playbooks, ledger, dataPlatform, telemetry, devops, autonomous }, null, 2) + '\n');
  console.log(`v15_build_complete events=${events.length} stages=${ledger.length} risks=${risks.length} score=${scores.intelligenceScore} autonomous=${scores.autonomousOpsScore} data_score=${scores.dataPlatformScore} devops_score=${scores.devopsOptimizationScore}`);
}

function lintAll() {
  const required = ['reports/v15-intelligence/evidence.json','reports/v15-data-platform-apps.json','reports/v15-data-platform-apps.ndjson','reports/v15-platform-telemetry.json','reports/v15-platform-telemetry.ndjson','reports/v15-devops-optimization.json','reports/v15-devops-optimization.ndjson','reports/v15-autonomous-ops.json','reports/v15-autonomous-ops.ndjson','reports/v15-risk-score.json','reports/v15-recovery-playbooks.json','reports/v15-observability.ndjson','reports/v15-prometheus-metrics.prom','reports/v15-grafana-dashboard.json','reports/v15-kibana-dashboard.ndjson','reports/v15-portal/index.html','reports/v15-portal/evidence.json'];
  ...
  for (const needle of ['jenkins-v15-autonomous-es', 'state-timeline', 'status-history', 'nodeGraph', 'Big Data App Readiness', 'Big Data Flow State', 'Autonomous Dimension Score', 'Incident Simulation Readiness', 'Data Quality Gate Score', 'Telemetry Upgrade Score', 'DevOps Node Pod Distribution']) if (!dashboardText.includes(needle)) throw new Error(`grafana missing ${needle}`);
  ...
  for (const needle of ['zhanglab-platform-pipeline-observability', 'zhanglab-platform-observability-universal-command', 'Universal Observability Command', '通用流水线评分总览', '通用自治运维维度', '通用 DevOps 节点分布', '通用 DevOps 优化台账', '通用大数据应用就绪度', '通用大数据链路证据']) if (!kibanaText.includes(needle)) throw new Error(`kibana missing ${needle}`);
  const html = fs.readFileSync('reports/v15-portal/index.html', 'utf8');
  for (const needle of ['v15-intelligence-hypergrid-20260601', 'v15-sonic-command-20260601', 'sound-rev-v2', 'Autonomous <span>DataOps Command', 'Autonomous Ops Dimensions', 'Incident Simulation Matrix', 'Data Quality Gates', 'Big Data Application Lab', 'Data Flow Evidence Map', 'DevOps Optimize', 'DevOps Node Pressure', 'Stateless Migration Candidates', 'Risk Radar', 'Recovery Room', 'Command Matrix', 'AudioContext', 'Sound ON', 'pointerdown', 'keydown']) if (!html.includes(needle)) throw new Error(`portal missing ${needle}`);
